    ...
    <div class="mb-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/60 dark:bg-gray-900/20 p-3">
        ...
        <div class="mt-2 grid grid-cols-2 lg:grid-cols-4 gap-2">
            ...
            <div class="rounded-lg border border-emerald-200 dark:border-emerald-800 bg-emerald-50/70 dark:bg-emerald-900/20 px-2.5 py-2">
                <div class="text-[10px] uppercase tracking-wide text-emerald-700/80 dark:text-emerald-300/80">Toplam Prim</div>
                <div class="text-sm font-semibold text-emerald-700 dark:text-emerald-300" x-text="formatPara(toplamPrimTutari)"></div>
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
            <thead class="bg-blue-50 dark:bg-blue-900/20">
                <tr>
                    ...
                    <th class="px-3 py-2.5 text-right text-[11px] font-bold text-emerald-700 dark:text-emerald-400 whitespace-nowrap">TOPLAM PRIM</th>
                </tr>
            </thead>
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                <template x-for="row in satirlar()" :key="row.hacizci_id">
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                        ...
                        <td class="px-3 py-2.5 text-right font-bold text-emerald-700 dark:text-emerald-400" x-text="formatPara(row.toplam_prim_tutari ?? 0)"></td>
                    </tr>
                </template>
            </tbody>
        </table>
...
function primListesiTab() {
    return {
        ...
        pivotData: {},
        aylar: ['Ocak', '�?ubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık'],
        ...
        get toplamPrimTutari() {
            return this.satirlar().reduce((toplam, row) => {
                const deger = parseFloat(row?.toplam_prim_tutari ?? 0);
                return toplam + (Number.isFinite(deger) ? deger : 0);
            }, 0);
        },
